...
public:
	SIMDSoAVector2	(size_t esize, value_type val)	: _x(esize, val), _y(esize, val) {}
	SIMDSoAVector2	(size_t esize)					: _x(esize), _y(esize) {}
	SIMDSoAVector2	()								: _x(), _y() {}
	...
	void push_back  (const math::Vector2<value_type>& val)
	{
		_x.push_back(val.x); _y.push_back(val.y);
	}

	void set		(size_t index, const math::Vector2<value_type>& val)
	{
		_x[index] = val.x;
		_y[index] = val.y;
	}

	const math::Vector2<value_type> get(size_t index) const
	{
		return math::Vector2<value_type>(_x[index], _y[index]);
	}

	const math::Vector2<value_type> operator[](size_t index) const
	{
		return math::Vector2<value_type>(_x[index], _y[index]);
	}
