...
template < class FTy_, class SCTy_, class NPTy_ >
typename SpatialNoiseGenerator<FTy_, SCTy_, NPTy_>::value_type 
	SpatialNoiseGenerator<FTy_, SCTy_, NPTy_>::operator()(const Vector2Type& vec) const
{
	...
	SpatialNoiseGenerator<FTy_, SCTy_, NPTy_>::operator()(value_type x, value_type y, value_type z) const
{
	...
	return ((*(data->sc))(data->exponents, &data->noise, data->lacunarity, 
		data->octaves, x, y, z) - data->center3) * data->scale3;
}

template < class FTy_, class SCTy_, class NPTy_ >
typename SpatialNoiseGenerator<FTy_, SCTy_, NPTy_>::value_type 
	SpatialNoiseGenerator<FTy_, SCTy_, NPTy_>::operator()(value_type x, value_type y) const
{
	const ThreadData* data = &_threadData[0];
	return ((*(data->sc))(data->exponents, &data->noise, data->lacunarity, 
		data->octaves, x, y) - data->center2) * data->scale2;
}
		  ...
		  value_type vscale = 1) const
{
	...
	else
	{
		...
		for( ; threadIdx < _threadCount - 1; ++threadIdx, totalElements += elementsPerThread)
		{
			...
			if(_useSSE)
			{
				ThreadPtr newThread(new boost::thread(std::bind(&this_type::callSSE, 
...
#endif
			{
				ThreadPtr newThread(new boost::thread(std::bind(&this_type::callNonSSE, 
					this, elementsPerThread, &results[totalElements], 
					&v.x(totalElements), &v.y(totalElements), &v.z(totalElements), 
					vscale, &_threadData[threadIdx])));
				threads.push_back(newThread);
													 ...
													 value_type vscale = 1) const
{
	...
	if(_threadCount == 1)
		asynchCalculate(static_cast<unsigned int>(v.size()), &results[0], 
		&v.x(0), &v.y(0), vscale, &_threadData[0]);
	else
	{
		// resize thread data array so each thread can have its own copy
		while(_threadData.size() < _threadCount)
		...
		for( ; threadIdx < _threadCount - 1; ++threadIdx, totalElements += elementsPerThread)
		{
			...
			if(_useSSE)
			{
				ThreadPtr newThread(new boost::thread(std::bind(&this_type::callSSE, 
					this, elementsPerThread, &results[totalElements],
					&v.x(totalElements), &v.y(totalElements), 
					vscale, &_threadData[threadIdx])));
				threads.push_back(newThread);
			}
			else
#endif
			{
				...
				//	&v.x(totalElements), &v.y(totalElements), 
				//	vscale, &_threadData[threadIdx])));
				ThreadPtr newThread(new boost::thread([&]() {
					callNonSSE(elementsPerThread, &results[totalElements], 
						&v.x(totalElements), &v.y(totalElements), 
						vscale, &_threadData[threadIdx]);
				}));
				threads.push_back(newThread);
...
template < class FTy_, class SCTy_, class NPTy_ >
typename SpatialNoiseGenerator<FTy_, SCTy_, NPTy_>::value_type 
	SpatialNoiseGenerator<FTy_, SCTy_, NPTy_>::operator()(value_type x, value_type y, const ThreadData* data) const
{
	return ((*data->sc)(data->exponents, &data->noise, data->lacunarity, 
		data->octaves, x, y) - data->center2) * data->scale2;
}
...
#ifdef ENABLE_SSE
	if(_useSSE)
		callSSE(count, results, x, y, vscale, data);
	else
#endif
		callNonSSE(count, results, x, y, vscale, data);
}
														   ...
														   value_type vscale, const ThreadData* data ) const
{
	...
	for(unsigned int i=0; i<count; ++i)
	{
		results[i] = (*this)(x[i] * vscale, y[i] * vscale, data);
	}
}
...
{
	_threadData[0].scale2 = 1.0;
	_threadData[0].center2 = 0.0;
	_threadData[0].scale3 = 1.0;
	_threadData[0].center3 = 0.0;
	value_type minh2=std::numeric_limits<value_type>::max(), 
		maxh2=std::numeric_limits<value_type>::min(),
		minh3=std::numeric_limits<value_type>::max(), 
		maxh3=std::numeric_limits<value_type>::min();
	for(unsigned int i = 0; i < 1000; ++i)
	{
		{
			value_type rx = static_cast<value_type>(rand()/static_cast<value_type>(RAND_MAX));
			value_type ry = static_cast<value_type>(rand()/static_cast<value_type>(RAND_MAX));
			value_type h = (*this)(math::Vector2<value_type>(rx, ry).normal());
			minh2 = MY_MIN(h, minh2);
			maxh2 = MY_MAX(h, maxh2);
		}
		{
			...
			minh3 = MY_MIN(h, minh3);
			maxh3 = MY_MAX(h, maxh3);
		}
	}

	_threadData[0].scale2 = 1 / (maxh2 - minh2);
	_threadData[0].center2 = static_cast<value_type>((maxh2 + minh2) * 0.5);
	_threadData[0].scale3 = 1 / (maxh3 - minh3);
	_threadData[0].center3 = static_cast<value_type>((maxh3 + minh3) * 0.5);
}
